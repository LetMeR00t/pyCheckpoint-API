...
@responses.activate
def test_add_lsm_gateway(management, resp_lsm_gateway):
    ...
    resp = management.network_objects.lsm_gateway.add(
        name="lsm_gateway",
...
@responses.activate
def test_show_lsm_gateway(management, resp_lsm_gateway):
    ...
    resp = management.network_objects.lsm_gateway.show(
...
@responses.activate
def test_set_lsm_gateway(management, resp_lsm_gateway):
    ...
    resp = management.network_objects.lsm_gateway.set(
        uid="f3b6df08-d973-4f16-8cfb-1f9562c6d120",
    ...
    with pytest.raises(MandatoryFieldMissing):
        management.network_objects.lsm_gateway.set()


@responses.activate
def test_delete_lsm_gateway(management, resp_message_ok):
    ...
    resp = management.network_objects.lsm_gateway.delete(
        uid="f3b6df08-d973-4f16-8cfb-1f9562c6d120"
    )
...
@responses.activate
def test_show_lsm_gateways(management, resp_from_to_objects):
    responses.add(
        responses.POST,
    ...
    resp = management.network_objects.lsm_gateway.show_lsm_gateways()

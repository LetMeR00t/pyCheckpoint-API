...
@responses.activate
def test_add_host(management, resp_host_ipv4, resp_host_ipv6):
    ...
    resp = management.network_objects.host.add(
        name="New Host 4",
    ...
    with pytest.raises(MandatoryFieldMissing):
        management.network_objects.host.add(name="New Host 4")


@responses.activate
def test_show_host(management, resp_host_ipv4):
    ...
    resp = management.network_objects.host.show(
    ...
    resp = management.network_objects.host.show(name="New Host 4")
    ...
    with pytest.raises(MandatoryFieldMissing):
        management.network_objects.host.show()


@responses.activate
def test_set_host(management, resp_host_ipv4, resp_host_ipv6):
    ...
    resp = management.network_objects.host.set(
        uid="9423d36f-2d66-4754-b9e2-e7f4493756d4",
        new_name="New Host 4",
        ip_address="192.0.2.1",
        tags=["t1", "t2", "t3"],
    ...
    with pytest.raises(MandatoryFieldMissing):
        management.network_objects.host.set()


@responses.activate
def test_delete_host(management, resp_message_ok):
    ...
    resp = management.network_objects.host.delete(
        uid="9423d36f-2d66-4754-b9e2-e7f4493756d4"
    )

    assert resp.message == "OK"

    resp = management.network_objects.host.delete(name="New Host 4")

    assert resp.message == "OK"

    with pytest.raises(MandatoryFieldMissing):
        management.network_objects.host.delete()


@responses.activate
def test_show_hosts(management, resp_from_to_objects):
    responses.add(
        responses.POST,
    ...
    # Show partial hosts
    resp = management.network_objects.host.show_hosts(

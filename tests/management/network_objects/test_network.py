...
@responses.activate
def test_add_network(management, resp_network_ipv4, resp_network_ipv6):
    ...
    resp = management.network_objects.network.add(
        name="New Network 4",
        subnet="192.0.2.0",
    ...
    with pytest.raises(MandatoryFieldMissing):
        management.network_objects.network.add(name="network1")
    # Missing mask information
    with pytest.raises(MandatoryFieldMissing):
        management.network_objects.network.add(name="network1", subnet="192.0.2.0")


@responses.activate
def test_show_network(management, resp_network_ipv4):
    ...
    resp = management.network_objects.network.show(
...
@responses.activate
def test_set_network(management, resp_network_ipv4, resp_network_ipv6):
    ...
    resp = management.network_objects.network.set(
    ...
    with pytest.raises(MandatoryFieldMissing):
        management.network_objects.network.set()


@responses.activate
def test_delete_network(management, resp_message_ok):
    ...
    resp = management.network_objects.network.delete(
        uid="d5e8d56f-2d77-4824-a5d2-c4a7885dd4a7"
    )
...
@responses.activate
def test_show_networks(management, resp_from_to_objects):
    responses.add(
        responses.POST,
    ...
    resp = management.network_objects.network.show_networks()

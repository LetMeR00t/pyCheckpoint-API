...
@responses.activate
def test_add_access_layer(management, resp_access_layer):
    ...
    resp = management.access_control_nat.access_layer.add(
        name="New Layer 1",
...
@responses.activate
def test_show_access_layer(management, resp_access_layer):
    ...
    resp = management.access_control_nat.access_layer.show(
    ...
    resp = management.access_control_nat.access_layer.show(name="New Layer 1")
...
@responses.activate
def test_set_access_layer(management, resp_access_layer):
    ...
    resp = management.access_control_nat.access_layer.set(
        name="New Layer 1",
        add_default_rule=True,
        applications_and_url_filtering=False,
        content_awareness=False,
...
@responses.activate
def test_delete_access_layer(management, resp_message_ok):
    ...
    resp = management.access_control_nat.access_layer.delete(
...
@responses.activate
def test_show_access_layers(management, resp_from_to_objects):
    responses.add(
        responses.POST,
    ...
    resp = management.access_control_nat.access_layer.show_access_layers(
        offset=0,
        limit=20,
        order={"ASC": "name"},
